...
function refreshDiagnostics(doc, collection) {
    if (doc.languageId !== "lc2k")
        return;
    const diags = [];
    const labels = new Map();
    const seenFillsAfterInstr = { firstFill: null, laterInstrs: [] };
    ...
    for (let i = 0; i < doc.lineCount; i++) {
        ...
        if (have < needRegs) {
            ...
            function isReg(t) { return /^[0-7]$/.test(t); }
            function isNumOrLabel(t) {
...
function makeDiag(doc, line, start, end, msg) {
