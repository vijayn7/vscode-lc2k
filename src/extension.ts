...
const INSTRUCTION_INFO: { [key: string]: InstructionInfo } = {
  "add": {
    type: "R-type",
...
export function activate(context: vscode.ExtensionContext) {
  ...
  const hoverProvider = vscode.languages.registerHoverProvider("lc2k", {
    provideHover(document, position, token) {
      const config = vscode.workspace.getConfiguration("lc2k");
      if (!config.get<boolean>("showInstructionHints", true)) {
        return;
      }
...
function generateInstructionReferenceHtml(): string {
  const instructionRows = Object.entries(INSTRUCTION_INFO).map(([opcode, info]) => 
    `<tr>
...
function assembleLc2k(assemblyCode: string): string {
